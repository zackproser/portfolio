...
it answer as me - or at least as my writing. Let's see how I do this in my server-side API route.
...
export async function POST(req: Request) {
  ...
  // Get the context from the last message, specifying that we want to use a maxtokens value of 3000
  // and we're only interested in items scored as 0.8 relevance or higher by Pinecone
...
The `getContext` function is a wrapper around the `getEmbeddings` and `getMatchesFromEmbeddings` functions, which are also defined in separate 'services' files: 
...
We use our context service to convert the user's query into a query embedding and query our Pinecone vector database for related documents, which are chunks of blog posts. 
