...
4. You can hook your build scripts either into npm's `prebuild` hook, which runs before `npm run build` is executed, or to the `pnpm` build target, so that your data driven pages are freshly rebuilt 
...
This function generates the full markdown content of the blog post, including the frontmatter metadata, introduction, table of contents, tool table, and category sections. By breaking this out into a separate function, we can focus on the high-level structure of the post without getting bogged down in the details of how each section is generated.
...
This code does a few important things:
    ...
    It determines the correct directory and filename for the generated page based on the project structure.
    It checks if the file already exists and, if so, extracts the existing date from the frontmatter. This allows us to preserve the original publication date if we're regenerating the page.
...
Creating data-driven pages helps you create rich, informative content that is easy to maintain and update over time. 
