...
- A→Z video walkthroughs (design → ingest → deploy)
- Robust notebook with sanity checks, failure modes, and fixes
- Next.js app wired for streaming, citations, related-content UI
- CI/CD that embeds only changed files on push
- Env patterns for OpenAI/Pinecone + index lifecycle
- Troubleshooting matrix (timeouts, dim mismatch, context blowups, rate limits)
- Production checklist (evals, logging, fallbacks, guardrails)
