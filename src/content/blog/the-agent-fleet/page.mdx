...
# The Agent Fleet That Runs My Business: a Claude Tag Over an eve Crew
...
The same move covers the unhappy path. When Margin's scheduling logic is too eager, I don't open Margin's repo at my desk. I tag `@Claude` in Slack from wherever I am, point it at Margin, and it reads the repo, finds the bug, and opens a PR.

{/* TODO: Slack recovery screenshot (image.png) — Zack to drop directly; shows Claude helping recover from a failure in Slack */}
...
> **Tools / APIs:** Notion, LinkedIn, Linear, GitHub, Stripe / billing (admin-governed Slack connections, scoped per channel); Wispr Flow (dictation)
>
> **What it does:** Sits over the whole fleet as the meta-agent and reaches the control surfaces I run on through admin-governed Slack connections, but only the ones I've granted in that channel. From one thread it triages email, updates the blog, opens PRs against the portfolio, and rebuilds the other bots themselves. It's the agent that maintains the agents.
...
> **Chief of Staff — Tools / APIs:** Linear (workflow + task system of record), Notion (shared knowledge base), eve remote-agent calls over `POST /eve/v1/session`
>
> **What it does:** The primary surface I drive the whole operation from — I describe an outcome once and it decides which specialists to wake, in what order, and threads their results back into one answer. It tracks every workflow in Linear and grounds its decisions in the same Notion knowledge base the rest of the fleet shares, so anything I kick off has a record I can check, reprioritize, or hand off. It still doesn't reimplement the specialists — it coordinates them, so the complexity stays in the orchestrator instead of landing on me.
...
> **Tools / APIs:** Cal.com, Google Calendar, Oura ring API
>
> **What it does:** Owns time and calendar for the whole operation, reading my Oura readiness and sleep scores each morning and holding childcare blocks as immovable. It hands leads a Cal.com link that only ever offers genuinely open slots across my personal and work calendars, so meetings book themselves with no back-and-forth. If I say I need a break, it checks whether the day can afford it and drops a real 2-4 hour rest block on my calendar.
...
> **Tools / APIs:** Gmail (triaging the `zachary@mindonfire.net` business mailbox)
>
> **What it does:** Reads Gmail, classifies what comes in, and drafts replies against a real business mailbox instead of my personal inbox. A lead lands as a structured summary, a suggested reply, and a proposed next action, often a Margin calendar link, so answering is one approval, not a context-switch. It's built to own and schedule the follow-ups I always mean to send three days later.
...
> **Tools / APIs:** Resend API
>
> **What it does:** Runs email marketing and sends through the Resend API, with one defining feature: a hard outbound kill-switch, so nothing reaches a real inbox until I explicitly approve it. It keys every campaign on `campaignId + approvedRevision` so an approved send can never double-fire. The whole bot is built so a mistake costs a redraft, not 5,000 misfired emails.
...
> **Tools / APIs:** Nano Banana Pro (image generation), Veo API (animation)
>
> **What it does:** Generates all the pixel art on the site in one consistent house style, so every post looks like it came from the same studio. Blog hands it a scene and Image returns the art — drawn with Nano Banana Pro and animated through the Veo API when a piece needs to move. No stock photos, no clip-art: one bot, one visual identity.
...
> **Tools / APIs:** Plaid (on Chase Business today), Mercury API (after the bank move)
>
> **What it does:** Keeps the books, reading account activity through Plaid on top of Chase Business to reconcile the bank against the ledger, and reads transactions only, it never moves money. Once banking moves to Mercury it plugs straight into Mercury's API instead. Invoicing from a Slack sentence is the half it's growing into, tracked against incoming transactions until paid.
...
> **Tools / APIs:** DocuSign, Google Drive
>
> **What it does:** Mints uniform business artifacts (SOWs, MSAs, NDAs, order forms) from templates so every contract carries the same clauses and never fat-fingers a client name or term. I describe the engagement in Slack, approve the draft, and it goes out through DocuSign for signature. When the signed copy returns it files it in the right Google Drive client folder, named consistently, where I can find it years later.
...
> **Tools / APIs:** GitHub (opens the PR); internal hand-off to Image
>
> **What it does:** Researches and drafts each post, asks Image for the art and diagrams, then runs its own voice and anti-slop passes before anything ships. When a draft is ready it opens a pull request against my portfolio repo on GitHub, so a post arrives as a reviewable diff, never a surprise on the live site. Nothing publishes until I merge.
...
> **Social — Tools / APIs:** Typefully API
>
> **What it does:** Rewrites a finished post into platform-native social copy, then publishes or schedules it through Typefully, which fans one draft out to five feeds in a single shot: LinkedIn, X, Mastodon, Bluesky, and Threads. One draft, one publish, five networks.

> **Portfolio — Tools / APIs:** Ahrefs (SEO + site health), Plausible (analytics)
>
> **What it does:** Owns site operations, watching SEO and site health through Ahrefs and reading traffic through Plausible, so growth and any broken-link or ranking regressions surface as something I can act on instead of something I learn about months late. It keeps the front door working while the writing bots fill the rooms.

Social stages posts to LinkedIn, X, Mastodon, Bluesky, and Threads through Typefully, and Portfolio handles site ops, SEO, and analytics. Both run today and need the least from the vision. They're the proof that a bot doesn't have to be ambitious to be worth it. Some of the fleet exists to keep the lights on while the rest does the heavy lifting.
...
## The point was never the bots. It's the surface area.

None of this took a research lab — it's Claude, a handful of named agents, and a phone to drive them from. The fleet doesn't replace me; it gives one person a surface area they have no right to. I write the post, and the fleet ships it, illustrates it, pushes it to five networks, and pings me the moment something breaks.

That's the whole game now: agents scale infinitely, but your attention doesn't. The teams that win won't be the ones with the most agents, they'll be the ones who learn to point a fleet at the right work and then get out of its way.

<div className="not-prose my-10 rounded-2xl border border-zinc-200 bg-zinc-50 p-6 dark:border-zinc-700/60 dark:bg-zinc-800/40">
  <h3 className="mb-1 text-xl font-semibold text-zinc-900 dark:text-zinc-100">Want to run your own fleet?</h3>
  <p className="mb-5 text-sm text-zinc-600 dark:text-zinc-400">Where I'm headed next, how I'm building it, and how to bring it to your team.</p>
  <ul className="m-0 list-none space-y-3 p-0">
    <li className="m-0">
      <Link href="/subscribe" className="font-semibold text-blue-600 no-underline hover:underline dark:text-blue-400">📬 The Modern Coding letter</Link>
      <span className="block text-sm text-zinc-600 dark:text-zinc-400">Where I publish what I'm building first. 5,000+ engineers.</span>
    </li>
    <li className="m-0">
      <Link href="https://youtube.com/watch?v=7qzgAkDN0q0" className="font-semibold text-blue-600 no-underline hover:underline dark:text-blue-400">▶️ Watch me build one</Link>
      <span className="block text-sm text-zinc-600 dark:text-zinc-400">Build Hermes AgentOS: 5 agents + a live mission-control dashboard.</span>
    </li>
    <li className="m-0">
      <Link href="/services" className="font-semibold text-blue-600 no-underline hover:underline dark:text-blue-400">🛠️ Bring it to your team</Link>
      <span className="block text-sm text-zinc-600 dark:text-zinc-400">Half-, full-, or two-day Claude enablement; we embed in your pods and leave you workflows, skills, and named champions.</span>
    </li>
    <li className="m-0">
      <Link href="/speaking" className="font-semibold text-blue-600 no-underline hover:underline dark:text-blue-400">🎤 Have me speak</Link>
      <span className="block text-sm text-zinc-600 dark:text-zinc-400">Agents scale infinitely, but your nervous system doesn't.</span>
    </li>
  </ul>
</div>
