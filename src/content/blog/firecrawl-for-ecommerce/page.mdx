...
for (const platform of reviewPlatforms) {
  ...
  const reviews = result.data.map(page => ({
    platform: platform,
    rating: extractRating(page.markdown),
    reviewText: extractReviewContent(page.markdown),
    reviewerProfile: extractReviewerInfo(page.markdown),
    helpful: extractHelpfulVotes(page.markdown),
    date: extractReviewDate(page.markdown)
  }))
}
```
