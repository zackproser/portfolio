...
```
Minimalist, flat vector illustration with clean lines and generous negative space.
Subject: a small paper boat floating in a shallow glass bowl of water on a wooden desk.
Lighting: soft, even studio lighting with a single subtle shadow; high contrast, no gradients.
Palette: 3–5 colors maximum; off-white background; muted blue water; natural wood tone; white paper boat.
Framing: centered composition, no text, no logos, no background clutter.
Style: professional product mock style; SVG-like crispness; no texture, no grain, no noise.
```

### 2) Animation prompt (Veo)

```
Animate the most obvious motion in the scene from the static source image of the paper boat in a bowl of water.
Make the boat gently bob and drift clockwise in a small loop as if moved by a faint ripple.
Maintain the minimalist style and color palette; do not introduce new objects or camera cuts.
No zoom, no pans; lock the camera; keep motion smooth and subtle.
Runtime target: ~3–4 seconds, seamless loop.
```
