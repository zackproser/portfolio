...
The encoder is the important part. A text embedding model tokenizes the input, runs it through a transformer, pools token representations into one vector, and often normalizes that vector. The same encoder must process both sides of a comparison. Coordinates from unrelated models live in unrelated spaces, even when their vectors have the same length.
