...
- Check Linear for tasks tagged "claw"
```
...
2. **I tag it with `claw`** — This signals it's delegated to my assistant
...
curl -s -X POST https://api.linear.app/graphql \
  ...
  -d '{"query": "{ issues(filter: { labels: { name: { eq: \"claw\" } }, state: { type: { nin: [\"completed\", \"canceled\"] } } }) { nodes { identifier title description } } }"}'
```
