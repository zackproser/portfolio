...
Teller lets you connect one account automatically from their dashboard, but after that you need to implement their Connect widget to link additional accounts. Claude Code was instrumental here. I had it spin up a local web server, implement the Teller Connect flow, and run it so I could authenticate my other bank accounts from a browser. Claude Code captured the enrollment tokens, I SSH'd them over to the Meerkat, and configured them in OpenClaw's environment — the same way anyone sets up credentials on a remote box. The whole process — from "I need to connect more accounts" to "all accounts are live on the Meerkat" — took about 20 minutes, most of which was me clicking through bank login screens.
