...
Concretely, here's what the manual review actually looks like for me. Every classification fires a preview email into my inbox: the original sender + subject, the assigned category and fit score, a one-line reasoning, and the proposed reply in a highlighted block. I open it, glance at the draft, jump to the Gmail thread, click Send. Each one takes maybe ten seconds.

<Image src="https://zackproser.b-cdn.net/images/inbound-classifier-preview-example.webp" alt="Preview email from the classifier showing an expert-network consultation request. Category badge reads 'expert-network · fit 8/10'. The proposed reply is highlighted: 'happy to take this one, AI agents is squarely in my wheelhouse. My rate is $650/hour. Happy to complete the standard compliance packet and get it on the calendar — send me the booking link or next steps. — Zack'" width={1200} height={833} />
...
<Image src="https://zackproser.b-cdn.net/images/inbound-classifier-gcloud-scopes.webp" alt="Google Cloud Console Data Access page showing the two Gmail scopes — gmail.send under 'Your sensitive scopes' with description 'Send email on your behalf', and gmail.modify under 'Your restricted scopes' with description 'Read, compose, and send emails from your Gmail account'" width={1200} height={875} />
...
The strict schema is the load-bearing piece: every field downstream (label routing, draft body, audit log) consumes the same parsed object, so one fragile prompt regression on field names breaks the rest. The `validateClassification` runtime check throws if the model drifts.
