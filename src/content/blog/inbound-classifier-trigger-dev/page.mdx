...
<Image src="https://zackproser.b-cdn.net/images/inbound-classifier-architecture.webp" alt="Pixel-art end-to-end architecture diagram: Gmail inbox with claude/needs-reply label → Trigger.dev cron task → Claude Opus classify + draft → Gmail draft created in the original thread → splits into manual-review path (Zack reviews and clicks Send) and auto-send path (only after a category graduates), with a Resend belt notification arrow on the side" width={1200} height={655} />
