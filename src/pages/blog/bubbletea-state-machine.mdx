...
Each stage therefore also has an Error. If the Error is ever not `nil`, then we effectively shut down the entire pipeline. This is also a design requirement: 
when we're orchestrating complex deployments, because we can't successfully run the `terraform apply` at the end if our `docker build` commands for the required images
...
does not need to be run again) which makes the pipeline speedier. 
...
machine that can smoothly orchestrate even complex processes with multiple different actors, tools, binaries and arguments - with zero noticeably UI lag or interruption 
