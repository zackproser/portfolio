...
import vercelFunctionsLogging from '@/images/vercel-functions-logging.png'
import vercelIntegratedAnalytics from '@/images/vercel-integrated-analytics.png'
...
Everytime I wanted to quicky pop back to my site after a long absence to fire off a quick blog post (something that sounds like it should be relatively easy) I ended up wrestling dependency mismatches between the various C packages needed to compile my SASS to CSS and the versions of node.js, Vue.js and nuxt.js that would interoperate correctly and still build all my custom (heavy) CSS files. Yikes. 
...
In skimming the docs for next.js and MDX, it appeared the project now supported that functionality and more out of the box with a nicer integration and less gross custom config required. In addition, it seemed that many rust-based tools to support key parts of the developer experience had replaced slower predecessors. I was intrigued enough to give it a shot. 
...
It feels like I stepped away for a couple years and then many of the UX pain points for developing on the frontend were handily resolved. To give some concrete examples, the way next.js handles things like fonts, prism for code highlighting which even supports custom themes (I installed gruvbox for my site, as you can see below, because it is the only correct choice) and MDX integration out of the box is exactly how I wanted to work with my blog content, because markdown is easy to maintain and quick to write, but I can now also render rich graphs or media to enrich my content. 
...
Once I had tweaked the template to my needs, I was mostly moving markdown in place, rewriting image links and adding some MDX / react preamble to my markdown files, which otherwise did not need to change. This was one of the least painful migrations I've done. 
...
I mean that literally. I'm slightly unnerved by how smooth of an experience deploying to Vercel was, because now I find myself wanting to launch tons of sites and projects there - which I'm certain is exactly their plan :)

<Image src={vercelIntegratedAnalytics} />
