...
export default function Paywall({ 
  ...
  console.log('Paywall props:', { price, slug, title, paywallHeader, paywallBody, buttonText });
  ...
  return (
    <div className="my-8 p-8 bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-900 rounded-xl shadow-xl border border-zinc-300 dark:border-zinc-700">
      <h3 className="text-2xl font-bold mb-6 text-center text-zinc-900 dark:text-zinc-100">
        {paywallHeader || "Ready to level up your development skills?"}
      </h3>

      <div className="text-center">
        <p className="mb-6 text-lg text-zinc-700 dark:text-zinc-300">
          {paywallBody || "Get instant access to the complete guide and start building powerful features today."}
        </p>
        <Button
        ...
        >
          {loading ? 'Processing...' : buttonText || `Get Instant Access ($${(price / 100).toFixed(2)})`}
        </Button>
      </div>
    </div>
  )
